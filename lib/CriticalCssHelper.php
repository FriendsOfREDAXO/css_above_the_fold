...
class CriticalCssHelper
{
    ...
    /**
     * Löscht eine spezifische Cache-Datei
     * 
     * @param string $viewport Der Viewport
     * @param int $article_id Die Artikel-ID
     * @param int $clang_id Die Sprach-ID
     ...
     */
    public static function deleteCacheFile($viewport, $article_id, $clang_id)
    {
        $cacheFile = self::getCacheFile($viewport, $article_id, $clang_id);
        
        if (file_exists($cacheFile)) {
            return rex_file::delete($cacheFile);
        }
    ...
    /**
     * Löscht alle Cache-Dateien für das AddOn
     * 
     * @return int Anzahl der gelöschten Dateien
     */
    public static function deleteAllCacheFiles()
    {
    ...
    /**
     * Liste aller Cache-Dateien abrufen
     * 
     * @return array Liste der Cache-Dateien mit Metadaten
     */
    public static function getCacheFiles()
    {
        $cacheDir = rex_path::addonCache('css_above_the_fold');
        $files = glob($cacheDir . '*.css');
        $cacheFiles = [];
        ...
        if (is_array($files)) {
            foreach ($files as $file) {
                ...
                $article_name = $article ? $article->getName() : 'Unbekannter Artikel';
                
                $clang = rex_clang::get($clang_id);
                $clang_name = $clang ? $clang->getName() : 'Unbekannte Sprache';
