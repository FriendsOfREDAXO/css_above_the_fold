 ...
 */
function getCacheFile($viewport, $article_id, $clang_id)
{
 ...
 */
function processWithInlineCss($cacheFile, $content)
{
    ...
    if ($addon->getConfig('load_css_async', true)) {
        ...
        $content = preg_replace_callback($regex, function ($matches) use (&$asyncCssBlock) {
            ...
            $asyncCssBlock .= getAsyncCssCode($cssUrl);
 ...
 */
function getAsyncCssCode($cssUrl)
{
 ...
 * @return string Modifizierter HTML-Content
 */
function processWithCriticalJs($viewport, $article_id, $clang_id, $content)
{
    ...
    $criticalJs = getCriticalJsCode($viewport, $article_id, $clang_id, $token);
 ...
 */
function getCriticalJsCode($viewport, $article_id, $clang_id, $token)
{
    $addon = rex_addon::get('css_above_the_fold');
    ...
    // Das JavaScript für die Critical CSS-Extraktion abrufen (in einer separaten Datei)
